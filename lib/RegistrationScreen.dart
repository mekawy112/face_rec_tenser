...
class _HomePageState extends State<RegistrationScreen> {
  ...
  doFaceDetection() async {
    ...
    if (faces.isNotEmpty) {
      if (faces.length == 1) {
        // If there is only one face, crop and register it directly
        cropAndRegisterFace(faces.first.boundingBox);
      } else {
        showFaceSelectionDialog();
      }
    } else {
  ...
  showFaceSelectionDialog() {
    showDialog(
      ...
      builder:
          (ctx) => AlertDialog(
            title: const Text("Select a Face", textAlign: TextAlign.center),
            content: SizedBox(
              height: 400,
              child: ListView.builder(
                shrinkWrap: true, // Added to fix intrinsic dimensions issue
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                    onTap: () {
                      cropAndRegisterFace(boundingBox);
